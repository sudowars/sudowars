...
/**
 * {@link RowRuleApplier} grants the possibility to apply a {@link Rule} to every row of a {@link FieldStructure}
 * @see RuleApplier
 * @see RuleApplier#applyRule(FieldStructure, Rule) 
 */
public class RowRuleApplier implements RuleApplier {

	/**
	 * Packs all used slots of row of {@link FieldStructure} in a {@link DependencyGroup} dependent on the given {@link Rule}.
	 * This way every row gets its own {@link DependencyGroup}.
	 ...
	 *
	 * @returns a {@link List} of {@link DependencyGroup}s where at every {@link DependencyGroup} represents a row.
	 *
	...
	public List<DependencyGroup> applyRule(FieldStructure fieldStructure,
		...
		List<DependencyGroup> grps = new ArrayList<DependencyGroup>(fieldStructure.getHeight());
		
		for (int y = 0; y < fieldStructure.getHeight(); y++) {
			List<Integer> row = new LinkedList<Integer>();
			for (int x = 0; x < fieldStructure.getWidth(); x++) {
				if (fieldStructure.isSlotUsed(x, y)) {
					row.add(fieldStructure.getIndex(x, y));
				}
			}
			grps.add(new DependencyGroup(rule, row));
		}
